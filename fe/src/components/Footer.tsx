...
import { Mail, Phone, MapPin, Globe, Shield, Terminal, ArrowUp } from "lucide-react";
...
export default function Footer({
  ...
  return (
    <footer id="global-footer" className="bg-slate-950 text-slate-400 py-16 border-t border-slate-900 relative select-none">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 pb-12 border-b border-slate-900 text-left">
          ...
          <div className="md:col-span-5 space-y-4">
            ...
            <div className="space-y-2.5 text-xs text-slate-500 pt-2">
              <p className="flex items-center">
                ...
                <span>Ho Chi Minh City | Silicon Valley, CA</span>
              </p>
              <p className="flex items-center">
          ...
          <div className="md:col-span-3 space-y-4">
            ...
            <ul className="space-y-2.5 text-xs">
              <li>
                <button onClick={() => setTab("home")} className="hover:text-blue-400 hover:underline transition-all">
                  {t.navHome}
                </button>
              </li>
              <li>
                <button onClick={() => setTab("solutions")} className="hover:text-blue-400 hover:underline transition-all">
                  {t.navSolutions}
                </button>
              </li>
              <li>
                <button onClick={() => setTab("services")} className="hover:text-blue-400 hover:underline transition-all">
                  {t.navServices}
                </button>
              </li>
              <li>
                <button onClick={() => setTab("about")} className="hover:text-blue-400 hover:underline transition-all">
                  {t.navAbout}
                </button>
              </li>
            </ul>
          ...
          <div className="md:col-span-4 space-y-4">
            <h4 className="font-display font-semibold text-white text-xs sm:text-sm uppercase tracking-wider">
              {language === "en" ? "Engineering Standards" : "Tiêu Chuẩn Lập Trình"}
            </h4>
            <div className="space-y-4 text-xs">
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-[11px] mt-0.5">High-security endpoints guarding sensitive corporate patient accounts.</p>
                </div>
              </div>
              <div className="flex items-start">
                <Terminal className="w-4 h-4 mr-2.5 text-blue-400 shrink-0 mt-0.5" />
                <div>
                  <p className="font-semibold text-white">Full Type-Safety Rigor</p>
                  <p className="text-slate-500 text-[11px] mt-0.5">Type-safe linted pipelines matching global enterprise source configurations.</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="pt-8 flex flex-col sm:flex-row justify-between items-center text-xs text-slate-600 gap-4">
          <p>© {new Date().getFullYear()} Korix Technology. All rights reserved. Built with precision.</p>
